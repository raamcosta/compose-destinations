...
internal class MermaidGraphWriter(
    ...
    private fun writeMermaidGraph(tree: RawNavGraphTree) {
        val name = tree.rawNavGraphGenParams.baseRoute
            .snakeToCamelCase()
        ...
        codeGenerator.makeFile(
            ...
            it += StringBuilder().apply {
                appendLine("---")
                appendLine("title: $name Navigation Graph")
                appendLine("---")
    ...
    private fun StringBuilder.appendGraphTreeLinks(tree: RawNavGraphTree) {
        Logger.instance.warn("writing links for ${tree.baseRoute}")
